 ...
 */
class fileReaderFleetOps implements pluginInterface {

	var $socket;
	var $channel = '';
	var $db = 'db/fileReaderFleetOps.db';
	var $lastCheck;
	var $config;

    function getDescription()
    {
    	$pluginName = "fileReaderFleetOps";
        $channels = array();
        $command = "";
        $description = "";
	...
	{
		$this->channel = $config["fileReader"]["fleetops"];
		$this->socket = $socket;
		$this->lastCheck = 0;
		$this->config = $config;
		if(!is_file($this->db))
